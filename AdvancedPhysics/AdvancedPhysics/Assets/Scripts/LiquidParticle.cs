...
public class LiquidParticle : MonoBehaviour
{
    ...
    const float WATER_DRAG = 1.0f;
    const float LAVA_DRAG = 0.3f;
    ...
    void Update()
    {
        switch (currentState)
        {
            ...
            case LiquidStates.Steam:
                if (GetComponent<Rigidbody>().velocity.y < 50)
                {
                    GetComponent<Rigidbody>().AddForce(new Vector2(0, STEAM_FLOATABILITY));
                }
                ScaleDown();
                break;
            default:
    ...
    public void SetState(LiquidStates newState)
    {
        ...
        switch (newState)
        {
            case LiquidStates.Lava:
                ...
                _rigidbody.drag = LAVA_DRAG;
                break;
            case LiquidStates.Water:
                renderer.material = waterMaterial;
                _rigidbody.drag = WATER_DRAG;
                break;
            case LiquidStates.Steam:
    ...
    void ScaleDown()
    {
        float scaleValue = startSize - ((Time.time - startTime) / particleLifeTime);
